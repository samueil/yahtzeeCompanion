...
const App = () => {
  ...
  const upperScore = CATEGORIES.UPPER.reduce((acc, cat) => acc + (scores[cat.id] || 0), 0);
  const bonus = upperScore >= 63 ? 35 : 0;
  const lowerScore = CATEGORIES.LOWER.reduce((acc, cat) => acc + (scores[cat.id] || 0), 0);
          ...
          <View className="z-10 flex-row items-center justify-between bg-primary-dark p-4 shadow-lg">
            <View>
              <View className="flex-row items-center">
                ...
                <Text className="ml-2 text-xl font-bold text-primary-light">Yahtzee Companion</Text>
              </View>
            ...
            <View className="mb-4 overflow-hidden rounded-lg border border-border bg-card">
              ...
              {CATEGORIES.UPPER.map((cat) => {
                ...
                return (
                  ...
                  >
                    <Text className={`font-medium ${taken ? 'text-disabled line-through' : 'text-text'}`}>
                      {cat.label}
                    </Text>
                    {taken ? (
            ...
            <View className="relative flex-row justify-center gap-2 bg-slate-50/90 px-2 pb-10 pt-6 backdrop-blur-md">
              {dice.map((val, idx) => (
                <View key={idx} className="relative items-center">
                  <View className={`h-16 w-16 ${locked[idx] ? 'scale-90 opacity-60' : 'scale-100'}`}>
                    <Suspense fallback={null}>
                      <Canvas camera={{ position: [0, 0, 5], fov: 45 }}>
                        <ambientLight intensity={3.5} />
                        <pointLight position={[10, 10, 10]} intensity={2.5} />
                        <pointLight position={[-10, -10, -10]} intensity={2.5} color="#ffffff" />
                        <ThreeDie value={val} isUiBlocked={isUiBlocked && !locked[idx]} />
                      </Canvas>
                    </Suspense>
                  </View>
                  <Pressable
                  ...
                  {rollsLeft < 3 && !locked[idx] && !isUiBlocked && (
                    <Pressable
                      onPress={(e) => { e.stopPropagation(); cycleDieValue(idx); }}
                  ...
                  {locked[idx] && (
                    <View className="absolute -top-2 right-0 z-50 rounded-full bg-primary px-2 py-0.5 shadow-sm">
                      <Text className="text-[8px] font-bold uppercase text-white">Locked</Text>
                    </View>
                  )}
                </View>
              ))}
            ...
            <View className="bg-white p-4 pt-0">
              {gameState === 'finished' ? (
                ...
                >
                  ...
                  <Text className="ml-2 text-lg font-bold text-white">New Game</Text>
                </Pressable>
              ) : (
                <View className="w-full flex-row items-center gap-4">
                  <Pressable
                    onPress={() => {
                      if (inputMode === 'camera') { setShowScanner(true); }
                      else { handleRandomRoll(); }
                    }}
                    ...
                    className={`h-16 flex-1 flex-row items-center justify-center gap-2 rounded-xl bg-primary shadow-lg active:opacity-90 ${
                      (rollsLeft === 0 || isUiBlocked) ? 'bg-slate-400 opacity-50' : ''
                    }`}
                  >
                    {inputMode === 'camera' ? <CameraIcon size={24} color={'white'} /> : <DicesIcon size={24} color={'white'} />}
                    <Text className="ml-2 text-lg font-bold text-white">
                      {rollsLeft === 3 ? 'Start Roll' : inputMode === 'camera' ? 'Scan Dice' : isUiBlocked ? 'Rolling...' : 'Roll Dice'}
                    </Text>
                  </Pressable>
